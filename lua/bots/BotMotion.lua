...
function BotMotion:OnGenerateMove(player)
            ...
            if updateMoveDir then
               ...
               if isStuck then
                ...
                elseif distToTarget <= 2.0 then
                    ...
                    local trace = Shared.TraceRay(eyePos, moveTargetPos, CollisionRep.Move, PhysicsMask.AllButPCs, EntityFilterOne(player))
                    if trace.fraction >= 1 then
                      self.currMoveDir = (moveTargetPos - currentPos):GetUnit()
                    else
                      -- no line of sight to target, do expensive pathing call
    ...
    elseif self.currMoveDir:GetLength() > 1e-4 then
        ...
        -- Look in move dir
        self.currViewDir = Vector(self.currMoveDir.x, self.currMoveDir.y, self.currMoveDir.z)
