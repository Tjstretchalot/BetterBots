...
local function PlayerWillLikelyCollideDuringMove(player, dir, debug)
  ...
  local trace = Shared.TraceBox(extents, player:GetOrigin(), player:GetOrigin() + dir * 2, CollisionRep.Default, PhysicsMask.AllButPCsAndRagdolls, LerkCollisionFilter(player))
  if debug then
...
local function TryWiggleViewDirForMovement(player, viewDir)
  ...
  -- first try wiggle up
  for i=1, 2 do
    local newViewDir = (viewDir + Vector(0, i * 0.5, 0)):GetUnit()
